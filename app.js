...
function route(name){
 document.querySelectorAll(".page-section").forEach(s=>s.classList.toggle("hidden-section",s.id!=="home"&&s.id!==name&&s.id!=="ads"&&!(name==="home"&&s.id==="ads")));
 if(name==="home"){window.scrollTo({top:0,behavior:"smooth"});$("ads").classList.remove("hidden-section")}
 else document.getElementById(name)?.scrollIntoView({behavior:"smooth"});
 if(name==="directory")renderDirectory();if(name==="categories")renderCategories();if(name==="requests")renderRequests();if(name==="admin"&&isAdmin)renderAdmin();
}
function routeFromHash(){let n=location.hash.slice(1)||"home";if(!["home","directory","categories","requests","about","list","post-request","admin"].includes(n))n="home";if(n==="admin"&&!isAdmin)n="home";route(n)}
document.addEventListener("click",e=>{
 const r=e.target.closest("[data-route]");if(r){e.preventDefault();let n=r.dataset.route;history.pushState({}, "", "#"+n);route(n)}
...
function updateAuthUI(){
 ...
 $("authToggle").classList.toggle("hidden",signed);$("adminNav")?.classList.toggle("hidden",!isAdmin);
}
